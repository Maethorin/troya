...
def main(args=None):
    ...
    parser.add_argument('--port', '-p', type=int, default="8000", help="Port to start the server with.")
    ...
    parser.add_argument('--debug', '-d', action='store_true', default=False, help='Indicates whether to run app web in debug mode.')
    ...
    application = App(config, log_level, options.debug)
    server = HTTPServer(application, xheaders=True)

    server.bind(options.port, options.bind)
    server.start(config.NUMBER_OF_FORKS)

    logging.info('-- Web started listening in %s:%d --' % (options.bind, options.port))
    try:
        tornado.ioloop.IOLoop.instance().start()
    except KeyboardInterrupt:
        logging.info('')
        logging.info('-- Web closed by user interruption --')
